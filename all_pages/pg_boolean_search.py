    ...
    if "picooptions" not in st.session_state:
        ...
        for attribute in st.session_state.attributes_list:
            if attribute.HasChildren:
                ...
                st.session_state.picooptions[attribute.AttributeName] = [list(freqs["Codes"]), attribute]
...
def year_filtering():
    ...
    yearfilter = st.toggle("Add Publication Year Filter", )
    if yearfilter:

        if st.session_state.year_histogram_df.shape[0] == 0:
            get_year_histogram()
        yrlist = sorted(st.session_state.year_histogram_df['Year'].unique(), reverse=True)
...
def boolsearcher():
    ...
    textsearch = st.toggle("Advanced text search", )
    if textsearch:
        st.text_input("Freetext Query",placeholder="Enter Freetext Query, eg. title:'AI equity'~5 AND abstract:(machine OR learn*)", key="freetext_query")
        ...
        if len(st.session_state.freetext_query) == 0:
            ...
            multi = st.multiselect("Optional: Select '{}' concepts (or leave blank and add search arm to select all)".format(picooption), st.session_state.picooptions[picooption][0], key="multiselect")
    ...
    def add_pico():
        ...
        colors_rgb = px.colors.sample_colorscale("Rainbow", [n / (n_cols - 1) for n in range(n_cols)])
        colors=[]
        for c in colors_rgb:
            colors.append(rgb_to_rgba(c, 0.3))

        coldict={val: colors[i] for i,val in enumerate(st.session_state.picooptions.keys())}
        ...
        if st.session_state.multiselect ==[] and st.session_state.freetext_query=="":
            ...
            st.session_state.multiselect = st.session_state.picooptions[picooption][0]#add all options
    ...
    def search_all(this_searchdoc):
        ...
        searchinfo=["The following searches were carried out on {}".format(formatted_date_time)]
        ...
        for i, sd in enumerate(st.session_state.searchdoc):#for each arm
            ...
            status_text.text(f"Processing arm {i + 1} of {len(st.session_state.searchdoc)}")
            ...
            for entry in sd:
                if entry[1] == "Free Text Query":
                    ...
                    for attribute in st.session_state.attributes_list:
                        if attribute.AttributeName==this_name and attribute.ParentAttributeId==this_parent.AttributeId:
                            selected_attributes.append(attribute)
                            print("Successfully identified parent for {}".format(this_name))
            ...
            if len(selected_attributes)>0:
                ...
                for indx, sir in enumerate(st.session_state.search_info_retrieval):
                    searchinfo.append(
                        "Searched <{}> and retreived {} results".format(selected_attributes[indx].AttributeName, sir))
                    # st.code(searchinfo[-1], language="markdown")
                ...
                if len(myquery)>0:
                    ...
                    searchinfo.append("Searched query <{}> on field <{}> and retrieved {} results".format(myquery, myfield, current_arm.shape[0]))

                else:
                    searchinfo.append("Warning: No Attribute and no queries found for this search")
            ...
            #st.code(searchinfo[-1], language="markdown")
            if len(dfs)>1:
                if st.session_state.operators[i]=="AND":#keep records for which item ids are in both dfs
                    source=dfs[0]
                    target=dfs[1]
                    ...
                    dfs=[merged]
                searchinfo.append("\nUsing [{}]: <{}> aggregated results between this and the previous arm.".format(st.session_state.operators[i],
                                                                                   dfs[0].shape[0]))
                #st.code(searchinfo[-1], language="markdown")

        if "filtered_years" in st.session_state:

            if len(st.session_state.filtered_years)>0:
                st.write("Registered year filter, this will be integrated in the next versio of the UI. Selected years: {}".format(", ".join(st.session_state.filtered_years)))
                # st.write("Applying publication year filtering...")
                # searchinfo.append("\n\n Year Filter applied:")
                # yearresults=[]
                # for y in st.session_state.filtered_years:
                #     yearresults.append(refs_per_textsearch(field="PubYear", query=y))
                #     searchinfo.append(
                #         "Filtered <{}> on publication years and retrieved {} results".format(y, "PubYear",
                #                                                                             yearresults[-1].shape[0]))
                # yrdf=pd.concat(yearresults)
                # yrdf=yrdf.drop_duplicates(subset=['itemId'], keep='first')
                # source = dfs[0]
                # dfs = [source[source['itemId'].isin(yrdf['itemId'])]]
